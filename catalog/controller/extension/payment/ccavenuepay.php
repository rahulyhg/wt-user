...
class ControllerExtensionPaymentCcavenuepay extends Controller {
	public function index() {
		...
		if ($order_info)		{
			...
			$Url 						= $this->url->link('extension/payment/ccavenuepay/callback');
			...
			if($order_info['shipping_firstname'])
			{
				...
				$delivery_country					= $order_info['payment_iso_code_2'];
			}
		
			$billing_email								= $order_info['email'];
			$merchant_param1							= $this->session->data['comment'];
			$redirect_url								= $this->url->link('extension/payment/ccavenuepay/callback');
			$cancel_url									= $this->url->link('extension/payment/ccavenuepay/callback');
			...
			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . 'extension/payment/ccavenuepay.tpl')) {
			 
				return $this->load->view($this->config->get('config_template') . 'extension/payment/ccavenuepay.tpl',$data);
			} else {
		
				return $this->load->view('extension/payment/ccavenuepay.tpl',$data);
			}
	...
	public function callback() 
	{
		...
		if($order_info)
		{
			...
			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . 'extension/payment/ccavenuepay_response.tpl')) {
					$template = $this->config->get('config_template') . 'extension/payment/ccavenuepay_response.tpl';
				} else {
					$template = 'extension/payment/ccavenuepay_response.tpl';
				}
